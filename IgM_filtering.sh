...
command="${module_load} && mkdir -p ${tmp_output_folder} && NXF_WORK=${nextflow_temp_folder} nextflow run -pool-size "'$SLURM_CPUS_ON_NODE'" -resume ${SCRIPT_DIR}/IgM_filtering.nf --input_folder ${input_folder}  --output_dir ${tmp_output_folder} --filter ${filter} "
